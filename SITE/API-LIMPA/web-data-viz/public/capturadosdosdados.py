...
def conectar_banco():
    return mysql.connector.connect(
        ...
        password="43589543Lu",     # Senha do usuário
        database="runguard"  # Nome do banco de dados
    )

# Variáveis globais para monitoramento de uptime e downtime
inicio_uptime = datetime.now()
duracao_downtime = timedelta(0)  # Duração do downtime
...
def capturar_uso_io():
    ...
    uso_io = {
        'bytes_lidos': io_disco.read_bytes,
        'bytes_escritos': io_disco.write_bytes,
        'io_total': io_disco.read_bytes + io_disco.write_bytes
    }
    return uso_io
...
def verificar_downtime():
    ...
    return duracao_downtime.total_seconds() / 60  # Downtime em minutos
...
def inserir_dados(uso_cpu, desvio_padrao_cpu, uso_memoria, desvio_padrao_memoria, uso_io, downtime, indice_estabilidade, eficiencia_io, taxa_recuperacao, fkEquipamento):
    ...
    """
    valores = (
        uso_cpu, uso_memoria, psutil.virtual_memory().used, uso_io['bytes_lidos'], uso_io['bytes_escritos'],
        0, 0, 0, 0, 0, 0, desvio_padrao_cpu, desvio_padrao_memoria, uso_io['io_total'],
...
def main():
    ...
    while True:
        ...
        print(f"I/O do Disco - Bytes Lidos: {uso_io['bytes_lidos']} | Bytes Escritos: {uso_io['bytes_escritos']}")
