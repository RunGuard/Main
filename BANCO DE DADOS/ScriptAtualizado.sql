...
CREATE DATABASE IF NOT EXISTS runguard;
USE runguard;
...
ALTER TABLE equipamento MODIFY COLUMN cpuVersao VARCHAR(255);

CREATE TABLE dado (
    idDado INT PRIMARY KEY AUTO_INCREMENT,
    cpuPercent DOUBLE,                     
    memoriaPercent DOUBLE,
    latencia_rede DOUBLE,
    memoriaUsada DOUBLE,                       
...
SELECT 
    ...
    CONCAT(d.latencia_rede, "%") AS "Porcentagem Latência",
...
select * from informacaoAlerta;

-- Remove a chave estrangeira
ALTER TABLE informacaoalerta DROP FOREIGN KEY fkDado;

-- Trunca a tabela
TRUNCATE TABLE dado;

-- Recria a chave estrangeira (ajustando para o nome correto)
ALTER TABLE informacaoalerta ADD CONSTRAINT fkDado FOREIGN KEY (fkDado) REFERENCES dado(idDado);
